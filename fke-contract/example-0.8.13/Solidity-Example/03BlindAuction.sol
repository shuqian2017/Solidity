...
contract BlindAuction {
        ...
        onlyBefore(revealEnd)
    {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            (uint _value, bool _fake, bytes32 _secret) = (values[i], fake[i], secret[i]);
            if (_bid.blindedBid != keccak256(abi.encode(_value, _fake, _secret))) {
                // 出价未能正确披露
                // 不返还订金
                continue;
            }
            refund += _bid.deposit;
            if (!_fake && _bid.deposit >= _value) {
                // 判断竞价是否超过最高价成功
