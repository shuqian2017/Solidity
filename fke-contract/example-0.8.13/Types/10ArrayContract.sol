...
contract ArrayContract {
    ...
    // 注意下面的代码并不是一对动态数组
    // 而是一个数组元素为一对变量的动态数组（也就是数组元素长度为2的定长数组的动态数组）。
    // 因为 T[] 总是T的动态数组,尽管T是数组
    // 所有的状态变量的数据位置都是storage
    bool[2][] pairsOfFlags;

    // newPairs 存储在memory 中(仅当它是公有的合约函数)
    ...
    function f(uint[] memory c) public {
        ...
        // 同样改变了 `s.moreInfo`.
        g.moreInfo = 2;
        // 进行了拷贝,因此 `g.contents` 不是本地变量,而是本地变量的成员
        g.contents = c;
    }
